...
class ContractClassifierService:
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "num_classes": len(self.classes) if self.classes is not None and len(self.classes) > 0 else 0,
            "classes": list(self.classes) if self.classes is not None and len(self.classes) > 0 else [],
...
def test_classifier():
    ...
    for i, result in enumerate(batch_results):
        ...
        print(f"{status} Document {i+1}: {result['prediction']} (Confidence: {result['confidence']:.3f})")
        if result['error']:
